...
pub fn create_script_service(new_script: NewScript) -> Result<Script, Error> {
    ...
    script_file
        .find_closest_braces("const target = await browser.waitForTarget", false)
        .wrap_with_pattern("browser.once('targetcreated', async (target) => {\n...\n})")
        .rewrite_content();
    script_file
        .find_between_string("await puppeteer.Locator.race([", "])", false)
        .wrap_with_pattern("await (\n...\n)")
        .rewrite_content();
    script_file
        .find_closest_braces("browser.once('targetcreated', async (target)", false)
        .wrap_with_pattern(
        ...
        .rewrite_content();
    script_file
        .remove_lines("const target = await browser.waitForTarget")
        .remove_lines(".setTimeout(timeout)")
        .remove_lines(".on('action', () => startWaitingForEvents())")
        .remove_lines("startWaitingForEvents()")
        .replace_words("puppeteer.Locator", "Promise")
        .replace_words("targetPage.locator", "targetPage.waitForSelector")
        .replace_words(".fill(", ".type(");
    script_file
        .find_between_string("await (", ";", true)
        .filter_list_content(".type(")
        .wrap_with_pattern("await new Promise(r => setTimeout(r, 1000));\n...")
        .rewrite_content();
    script_file
        .find_closest_braces("const targetPage = await target.page()", false)
        .wrap_with_pattern(
            "try {
                    ...
        ...
        .rewrite_content();
    script_file
        .find_closest_braces("new Promise((resolve, reject)", true)
        .remove_brace()
        .rewrite_content();
...
pub fn update_script_service(
    script_id: String,
    updated_script: UpdateScript,
) -> Result<Script, Error> {
